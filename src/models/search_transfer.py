...
class SearchTransfer(nn.Module):
    ...
    def forward(self, query_lv3, key_lv3, value_lv1, value_lv2, value_lv3):
        """与RefGT对应的特征搜索和转移模块
        
        参数:
            query_lv3: 查询特征 (dehazed1的lv3特征)
            key_lv3: 键特征 (dehazed2的lv3特征)
            value_lv1: 值特征lv1 (clear的lv1特征)
            value_lv2: 值特征lv2 (clear的lv2特征)
            value_lv3: 值特征lv3 (clear的lv3特征)
        ...
        with torch.no_grad():
            ...
            # 转移阶段
            value_lv3_unfold = F.unfold(value_lv3, kernel_size=(kernel_size, kernel_size), padding=padding)
            value_lv2_unfold = F.unfold(value_lv2, kernel_size=(2*kernel_size, 2*kernel_size), 
                                          padding=2*padding, stride=2)
            value_lv1_unfold = F.unfold(value_lv1, kernel_size=(4*kernel_size, 4*kernel_size), 
